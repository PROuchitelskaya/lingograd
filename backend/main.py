...
# Клиент пингует раз в 15 секунд сам, без участия ученика. Но телефон,
# который положили на парту и заблокировали, замораживает таймеры в браузере,
# поэтому порог щедрый: три минуты молчания — это двенадцать пропущенных пингов.
SILENT_AFTER_S = 180            # соединение молчит дольше — считаем оборванным
...
async def ws_endpoint(ws: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            # телефон разбудили после долгой паузы: соединение живо, но уборщик
            # уже счёл его оборванным. Возвращаем в комнату и досылаем состояние,
            # иначе у ученика экран замрёт до перезагрузки страницы
            lst = hub.conns.get(code)
            if lst is not None and conn not in lst:
                lst.append(conn)
                await hub.send(ws, session.snapshot(
                    player=player, teacher=(conn.role == "teacher")))
            t = msg.get("t")
